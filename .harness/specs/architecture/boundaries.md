...
1. **入口层与业务解耦**：CLI / MCP 只做参数解析与注册，业务逻辑全部下沉 `internal/core`。未来加 Web/UI 入口时，只需新增一个入口适配器，不动核心。
...
### 入口层（cmd/litkit · internal/mcp）
- CLI 子命令参数解析、MCP 工具注册与参数绑定
- 不承载任何业务逻辑，不做数据加工
- 共享 `internal/sources/registry.go`，保证 CLI 与 MCP 注册同步
...
- **为什么 CLI 与 MCP 共享核心**：C8 接口一致性约束——禁止行为分叉。注册表 + 架构检查保证"一处业务，两处注册"。
