...
| `LITKIT_LLM_API_KEY`                               | 可选，LLM 引用评分 key（全局回落；推荐在 `.litkit/verifier_models.json` 每模型 `api_key` 配置） |
| `LITKIT_LLM_BASE_URL`                              | 可选，LLM 自托管 endpoint（全局回落；JSON 每模型 `base_url` 优先）                              |
